 ...
 * time: 2019/8/8
 **/
class GzhListAdapter (val context: Context, datas: MutableList<Datas>) :
        BaseQuickAdapter<Datas, BaseViewHolder>(R.layout.gzh_list_item, datas){
    override fun convert(helper: BaseViewHolder?, item: Datas?) {
        ...
        if (helper != null) {
            ...
            helper.setText(R.id.item_title, item.title)
                    ...
                    .setText(R.id.item_kind, if (item.superChapterName == null) item.chapterName else item.superChapterName +"/"+item.chapterName)
                    ...
                    .setImageResource(
                            R.id.item_collect,
                            if (item.originId != 0 || item.collect) R.drawable.like else R.drawable.no_like
                    )
